...
export class StoreRepository {
  ...
  private allowedFields:string[]=["id","created_at","name","type","location","lat","lng","rating","is_open","tags"];
  ...
  private cleanData(data:Partial<Store>){
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM store");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM store WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO store (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO store (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(id:string,data:Partial<Store>){
  ...
  async update_all(rows:any[] = [], data:Partial<Store>){
  ...
  async update_by(condition:any, data:Partial<Store>){
  ...
  async delete_all(){
    await this.execute("TRUNCATE store");
    this.cache.clear();
