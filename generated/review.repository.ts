...
export class ReviewRepository {
  ...
  private allowedFields:string[]=["id","order_id","user_id","store_id","rating","comment","tags"];
  ...
  private cleanData(data:Partial<Review>){
  ...
  private async exists(id:string){
    ...
    const query = "SELECT id FROM review WHERE id=?";
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM review");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM review WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO review (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO review (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(id:string,data:Partial<Review>){
  ...
  async update_all(rows:any[] = [], data:Partial<Review>){
  ...
  async update_by(condition:any, data:Partial<Review>){
  ...
  async delete_all(){
    await this.execute("TRUNCATE review");
    this.cache.clear();
