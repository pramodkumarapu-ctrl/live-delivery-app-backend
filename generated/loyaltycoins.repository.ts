...
import { LoyaltyCoins, User } from "./types";
...
@Injectable()
export class LoyaltyCoinsRepository {

  private allowedFields:string[]=["user_id","transaction_at","points","reason"];
  ...
  private cleanData(data:Partial<LoyaltyCoins>){
  ...
  private async exists(user_id:string, transaction_at:string){

    const query = "SELECT user_id FROM loyaltycoins WHERE user_id=? AND transaction_at=?";
  ...
  async find_one(user_id:string, transaction_at:string){

    const query = "SELECT * FROM loyaltycoins WHERE user_id=? AND transaction_at=?";
  ...
  async find_cached(user_id:string, transaction_at:string){

    const key = "${user_id}-${transaction_at}";

    const cached = this.getCache(key);
    if(cached) return {message:"cache hit",data:cached};

    const result = await this.find_one(user_id, transaction_at);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM loyaltycoins");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM loyaltycoins WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );

      for(const r of res.rows){
        const key = "${r.user_id}-${r.transaction_at}";
        results.set(key,r);
      }
    }
...
async insert_one(data: LoyaltyCoins){
  ...
  // ✅ CLUSTER KEY
  if (!payload.transaction_at) {
    payload.transaction_at = new Date();
  }
...
const keys = Object.keys(payload);
  ...
  if (!keys.includes("transaction_at")) {
    keys.push("transaction_at");
  }
  

  const query = `INSERT INTO loyaltycoins (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO loyaltycoins (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(user_id:string, transaction_at:string,data:Partial<LoyaltyCoins>){

    if(!(await this.exists(user_id, transaction_at))){
    ...
    const query = `UPDATE loyaltycoins
      SET ${keys.map(k=>`${k}=?`).join(",")}
      WHERE user_id=? AND transaction_at=?`;
  ...
  async update_all(rows:any[] = [], data:Partial<LoyaltyCoins>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.user_id, r.transaction_at, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<LoyaltyCoins>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT user_id, transaction_at FROM loyaltycoins
  ...
  async delete_one(user_id:string, transaction_at:string){

    if(!(await this.exists(user_id, transaction_at))){
      return {message:"Record not found",deleted:false};
    }

    const query = "DELETE FROM loyaltycoins WHERE user_id=? AND transaction_at=?";

    await this.execute(
      query,
      [this.toPK(user_id) , transaction_at]
    );
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.user_id, r.transaction_at))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE loyaltycoins");
    this.cache.clear();
