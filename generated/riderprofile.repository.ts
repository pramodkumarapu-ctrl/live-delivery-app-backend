...
import { RiderProfile, User } from "./types";
...
export class RiderProfileRepository {
  soft_delete(rider_id: string) {
    throw new Error("Method not implemented.");
  }

  private allowedFields:string[]=["rider_id","vehicle_type","rating","total_orders","current_status","created_at"];
  ...
  private cleanData(data:Partial<RiderProfile>){
  ...
  private async exists(rider_id:string){

    const query = "SELECT rider_id FROM riderprofile WHERE rider_id=?";
  ...
  async find_one(rider_id:string){
    ...
    const query = "SELECT * FROM riderprofile WHERE rider_id=?";
  ...
  async find_cached(rider_id:string){
    ...
    const result = await this.find_one(rider_id);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM riderprofile");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM riderprofile WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
async insert_one(data: RiderProfile){
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO riderprofile (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO riderprofile (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(rider_id:string,data:Partial<RiderProfile>){

    if(!(await this.exists(rider_id))){
    ...
    const query = `UPDATE riderprofile
  ...
  async update_all(rows:any[] = [], data:Partial<RiderProfile>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.rider_id, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<RiderProfile>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT rider_id FROM riderprofile
  ...
  async delete_one(rider_id:string){
    ...
    const query = "DELETE FROM riderprofile WHERE rider_id=? ";
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.rider_id))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE riderprofile");
    this.cache.clear();
