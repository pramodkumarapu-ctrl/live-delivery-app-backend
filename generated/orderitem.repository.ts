...
export class OrderItemRepository {
  ...
  private allowedFields:string[]=["order_id","product_id","quantity","price"];
  ...
  private cleanData(data:Partial<OrderItem>){
  ...
  private async exists(order_id:string, product_id:string){

    const query = "SELECT order_id FROM orderitem WHERE order_id=? AND product_id=?";
  ...
  async find_one(order_id:string, product_id:string){

    const query = "SELECT * FROM orderitem WHERE order_id=? AND product_id=?";
  ...
  async find_cached(order_id:string, product_id:string){
    ...
    const result = await this.find_one(order_id, product_id);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM orderitem");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM orderitem WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
async insert_one(data: OrderItem){
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO orderitem (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO orderitem (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(order_id:string, product_id:string,data:Partial<OrderItem>){

    if(!(await this.exists(order_id, product_id))){
  ...
  async update_all(rows:any[] = [], data:Partial<OrderItem>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.order_id, r.product_id, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<OrderItem>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT order_id, product_id FROM orderitem
  ...
  async delete_one(order_id:string, product_id:string){

    if(!(await this.exists(order_id, product_id))){
      return {message:"Record not found",deleted:false};
    }

    const query = "DELETE FROM orderitem WHERE order_id=? AND product_id=?";

    await this.execute(
      query,
      [this.toPK(order_id) , product_id]
    );
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.order_id, r.product_id))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE orderitem");
    this.cache.clear();
