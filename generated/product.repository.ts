...
export class ProductRepository {
  ...
  private allowedFields:string[]=["id","store_id","created_at","name","price","category","is_available","is_veg","stock","add_ons"];
  ...
  private cleanData(data:Partial<Product>){
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM product");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM product WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO product (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO product (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(id:string,data:Partial<Product>){
  ...
  async update_all(rows:any[] = [], data:Partial<Product>){
  ...
  async update_by(condition:any, data:Partial<Product>){
  ...
  async delete_all(){
    await this.execute("TRUNCATE product");
    this.cache.clear();
