...
import { OrderETA, User } from "./types";
...
@Injectable()
export class OrderETARepository {

  private allowedFields:string[]=["order_id","last_updated","current_eta","traffic_level","weather_impact"];
  ...
  private cleanData(data:Partial<OrderETA>){
  ...
  private async exists(order_id:string, last_updated:string){

    const query = "SELECT order_id FROM ordereta WHERE order_id=? AND last_updated=?";
  ...
  async find_one(order_id:string, last_updated:string){

    const query = "SELECT * FROM ordereta WHERE order_id=? AND last_updated=?";
  ...
  async find_cached(order_id:string, last_updated:string){

    const key = "${order_id}-${last_updated}";

    const cached = this.getCache(key);
    if(cached) return {message:"cache hit",data:cached};

    const result = await this.find_one(order_id, last_updated);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM ordereta");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM ordereta WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );

      for(const r of res.rows){
        const key = "${r.order_id}-${r.last_updated}";
        results.set(key,r);
      }
    }
...
async insert_one(data: OrderETA){
...
const keys = Object.keys(payload);
  ...
  if (!keys.includes("last_updated")) {
    keys.push("last_updated");
  }
  

  const query = `INSERT INTO ordereta (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO ordereta (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(order_id:string, last_updated:string,data:Partial<OrderETA>){

    if(!(await this.exists(order_id, last_updated))){
  ...
  async update_all(rows:any[] = [], data:Partial<OrderETA>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.order_id, r.last_updated, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<OrderETA>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT order_id, last_updated FROM ordereta
  ...
  async delete_one(order_id:string, last_updated:string){

    if(!(await this.exists(order_id, last_updated))){
      return {message:"Record not found",deleted:false};
    }

    const query = "DELETE FROM ordereta WHERE order_id=? AND last_updated=?";

    await this.execute(
      query,
      [this.toPK(order_id) , last_updated]
    );
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.order_id, r.last_updated))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE ordereta");
    this.cache.clear();
