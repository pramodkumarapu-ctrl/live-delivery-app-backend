...
export class CartRepository {
  soft_delete(arg0: { user_id: string; product_id: string; }) {
    throw new Error("Method not implemented.");
  }

  private allowedFields:string[]=["user_id","created_at","product_id","quantity","added_at"];
  ...
  private cleanData(data:Partial<Cart>){
  ...
  private async exists(user_id:string, product_id:string){

    const query = "SELECT user_id FROM cart WHERE user_id=? AND product_id=?";
  ...
  async find_one(user_id:string, product_id:string){
  ...
  async find_cached(user_id:string, product_id:string){
    ...
    const result = await this.find_one(user_id, product_id);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM cart");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM cart WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO cart (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO cart (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(user_id:string, product_id:string,data:Partial<Cart>){

    if(!(await this.exists(user_id, product_id))){
  ...
  async update_all(rows:any[] = [], data:Partial<Cart>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.user_id, r.product_id, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<Cart>){
  ...
  async delete_one(user_id:string, product_id:string){
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.user_id, r.product_id))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE cart");
    this.cache.clear();
