...
export class UserAddressRepository {
  ...
  private allowedFields:string[]=["user_id","id","label","lat","lng","address_text","is_default"];
  ...
  private cleanData(data:Partial<UserAddress>){
  ...
  private async exists(user_id:string, id:string){

    const query = "SELECT user_id FROM useraddress WHERE user_id=? AND id=?";
  ...
  async find_one(user_id:string, id:string){

    const query = "SELECT * FROM useraddress WHERE user_id=? AND id=?";
  ...
  async find_cached(user_id:string, id:string){
    ...
    const result = await this.find_one(user_id, id);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM useraddress");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM useraddress WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
async insert_one(data: UserAddress){
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO useraddress (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO useraddress (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(user_id:string, id:string,data:Partial<UserAddress>){

    if(!(await this.exists(user_id, id))){
  ...
  async update_all(rows:any[] = [], data:Partial<UserAddress>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.user_id, r.id, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<UserAddress>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT user_id, id FROM useraddress
  ...
  async delete_one(user_id:string, id:string){

    if(!(await this.exists(user_id, id))){
      return {message:"Record not found",deleted:false};
    }

    const query = "DELETE FROM useraddress WHERE user_id=? AND id=?";

    await this.execute(
      query,
      [this.toPK(user_id) , id]
    );
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.user_id, r.id))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE useraddress");
    this.cache.clear();
