...
export class RefundRepository {
  ...
  private allowedFields:string[]=["id","created_at","payment_id","order_id","amount","reason","status"];
  ...
  private cleanData(data:Partial<Refund>){
  ...
  private async exists(id:string, created_at:string){
    ...
    const query = "SELECT id FROM refund WHERE id=? AND created_at=?";
  ...
  async find_one(id:string, created_at:string){
    ...
    const query = "SELECT * FROM refund WHERE id=? AND created_at=?";
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM refund");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM refund WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO refund (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO refund (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(id:string, created_at:string,data:Partial<Refund>){
  ...
  async update_all(rows:any[] = [], data:Partial<Refund>){
  ...
  async update_by(condition:any, data:Partial<Refund>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT id, created_at FROM refund
  ...
  async delete_one(id:string, created_at:string){
    ...
    const query = "DELETE FROM refund WHERE id=? AND created_at=?";
  ...
  async delete_all(){
    await this.execute("TRUNCATE refund");
    this.cache.clear();
