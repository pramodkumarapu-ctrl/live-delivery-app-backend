...
export class ChatMessageRepository {
  ...
  private allowedFields:string[]=["order_id","timestamp","sender_id","message","msg_type","is_read"];
  ...
  private cleanData(data:Partial<ChatMessage>){
  ...
  private async exists(order_id:string, timestamp:string){
    ...
    const query = "SELECT order_id FROM chatmessage WHERE order_id=? AND timestamp=?";
  ...
  async find_one(order_id:string, timestamp:string){
    ...
    const query = "SELECT * FROM chatmessage WHERE order_id=? AND timestamp=?";
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM chatmessage");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM chatmessage WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
async insert_one(data: ChatMessage){
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO chatmessage (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO chatmessage (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(order_id:string, timestamp:string,data:Partial<ChatMessage>){
    ...
    const query = `UPDATE chatmessage
  ...
  async update_all(rows:any[] = [], data:Partial<ChatMessage>){
  ...
  async update_by(condition:any, data:Partial<ChatMessage>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT order_id, timestamp FROM chatmessage
  ...
  async delete_one(order_id:string, timestamp:string){
    ...
    const query = "DELETE FROM chatmessage WHERE order_id=? AND timestamp=?";
  ...
  async delete_all(){
    await this.execute("TRUNCATE chatmessage");
    this.cache.clear();
