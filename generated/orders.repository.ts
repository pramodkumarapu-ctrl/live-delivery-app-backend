...
export class OrdersRepository {
    soft_delete(id: string, created_at: string) {
        throw new Error("Method not implemented.");
    }

  private allowedFields:string[]=["id","created_at","user_id","store_id","rider_id","status","total_price","delivery_fee","surge_fee","discount_amount","tax_amount","final_amount","payment_status","payment_id","otp","address_snapshot"];
  ...
  private cleanData(data:Partial<Orders>){
  ...
  private async exists(id:string, created_at:string){

    const query = "SELECT id FROM orders WHERE id=? AND created_at=?";
  ...
  async find_one(id:string, created_at:string){
    ...
    const query = "SELECT * FROM orders WHERE id=? AND created_at=?";
  ...
  async find_cached(id:string, created_at:string){
    ...
    const result = await this.find_one(id, created_at);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM orders");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM orders WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO orders (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO orders (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(id:string, created_at:string,data:Partial<Orders>){

    if(!(await this.exists(id, created_at))){
  ...
  async update_all(rows:any[] = [], data:Partial<Orders>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.id, r.created_at, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<Orders>){
  ...
  async delete_one(id:string, created_at:string){
    ...
    const query = "DELETE FROM orders WHERE id=? AND created_at=?";

    await this.execute(
      query,
      [this.toPK(id) , created_at]
    );
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.id, r.created_at))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE orders");
    this.cache.clear();
