...
import { QuickActionLog, User } from "./types";
...
@Injectable()
export class QuickActionLogRepository {

  private allowedFields:string[]=["order_id","timestamp","action_type"];
  ...
  private cleanData(data:Partial<QuickActionLog>){
  ...
  private async exists(order_id:string, timestamp:string){
    ...
    const query = "SELECT order_id FROM quickactionlog WHERE order_id=? AND timestamp=?";
  ...
  async find_one(order_id:string, timestamp:string){
    ...
    const query = "SELECT * FROM quickactionlog WHERE order_id=? AND timestamp=?";
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM quickactionlog");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM quickactionlog WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
async insert_one(data: QuickActionLog){
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO quickactionlog (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO quickactionlog (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(order_id:string, timestamp:string,data:Partial<QuickActionLog>){
    ...
    const query = `UPDATE quickactionlog
  ...
  async update_all(rows:any[] = [], data:Partial<QuickActionLog>){
  ...
  async update_by(condition:any, data:Partial<QuickActionLog>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT order_id, timestamp FROM quickactionlog
  ...
  async delete_one(order_id:string, timestamp:string){
    ...
    const query = "DELETE FROM quickactionlog WHERE order_id=? AND timestamp=?";
  ...
  async delete_all(){
    await this.execute("TRUNCATE quickactionlog");
    this.cache.clear();
