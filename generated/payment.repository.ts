...
export class PaymentRepository {
  ...
  private allowedFields:string[]=["id","paid_at","order_id","user_id","amount","method","status","transaction_ref"];
  ...
  private cleanData(data:Partial<Payment>){
  ...
  private async exists(id:string, paid_at:string){

    const query = "SELECT id FROM payment WHERE id=? AND paid_at=?";
  ...
  private buildConditions(condition:any){
    ...
    for(const [k,v] of Object.entries(condition)){
      ...
      clauses.push(`${k}=?`);
      values.push(k==="id" ? this.toPK(v) : v);
    }
  ...
  async find_one(id:string, paid_at:string){

    const query = "SELECT * FROM payment WHERE id=? AND paid_at=?";
  ...
  async find_cached(id:string, paid_at:string){

    const key = "${id}-${paid_at}";

    const cached = this.getCache(key);
    if(cached) return {message:"cache hit",data:cached};

    const result = await this.find_one(id, paid_at);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM payment");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM payment WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );

      for(const r of res.rows){
        const key = "${r.id}-${r.paid_at}";
        results.set(key,r);
      }
    }
...
const keys = Object.keys(payload);
  ...
  if (!keys.includes("paid_at")) {
    keys.push("paid_at");
  }
  

  const query = `INSERT INTO payment (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO payment (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(id:string, paid_at:string,data:Partial<Payment>){

    if(!(await this.exists(id, paid_at))){
  ...
  async update_all(rows:any[] = [], data:Partial<Payment>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.id, r.paid_at, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<Payment>){
  ...
  private async find_ids(condition:any): Promise<any[]>{
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT id, paid_at FROM payment
  ...
  async delete_one(id:string, paid_at:string){

    if(!(await this.exists(id, paid_at))){
      return {message:"Record not found",deleted:false};
    }

    const query = "DELETE FROM payment WHERE id=? AND paid_at=?";

    await this.execute(
      query,
      [this.toPK(id) , paid_at]
    );
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.id, r.paid_at))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE payment");
    this.cache.clear();
