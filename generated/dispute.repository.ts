...
export class DisputeRepository {
  ...
  private allowedFields:string[]=["id","order_id","user_id","issue_type","status","refund_amount"];
  ...
  private cleanData(data:Partial<Dispute>){
  ...
  private async exists(id:string){
    ...
    const query = "SELECT id FROM dispute WHERE id=?";
  ...
  async find_one(id:string){
    ...
    const query = "SELECT * FROM dispute WHERE id=?";
  ...
  async find_cached(id:string){
    ...
    const result = await this.find_one(id);
  ...
  async find_all(){
    const res = await this.execute("SELECT * FROM dispute");
  ...
  async find_by(condition:any){
    ...
    for(const q of queries){
      ...
      const res = await this.execute(
        `SELECT * FROM dispute WHERE ${q.clause} ALLOW FILTERING`,
        q.values
      );
...
const keys = Object.keys(payload);
  ...
  const query = `INSERT INTO dispute (${keys.join(",")})
...
async insert_many_batch(items: User[]) {
  ...
  for (const item of items) {
    ...
    const query = "INSERT INTO dispute (" + keys.join(",") + ") VALUES (" + keys.map(() => "?").join(",") + ")";
  ...
  async update_one(id:string,data:Partial<Dispute>){
  ...
  async update_all(rows:any[] = [], data:Partial<Dispute>){
    if(!rows.length) return {message:"No rows provided",count:0};

    await Promise.all(
      rows.map(r => this.update_one(r.id, data))
    );

    return {message:"update_all success",count:rows.length};
  }

  async update_by(condition:any, data:Partial<Dispute>){
  ...
  async delete_one(id:string){
    ...
    const query = "DELETE FROM dispute WHERE id=? ";
  ...
  async delete_by(condition:any){
    ...
    await Promise.all(
      rows.map(r => this.delete_one(r.id))
    );
  ...
  async delete_all(){
    await this.execute("TRUNCATE dispute");
    this.cache.clear();
